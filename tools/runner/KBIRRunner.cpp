...
/** The working memory
 */
static WorkingMemory *S_wm = NULL;
...
/** The Agenda.
 */
static Agenda *S_Agenda;

extern "C"
int ag_ppeval( char* txt ){
  std::string goal( txt );
  int ret = S_Agenda->push( goal, S_wm );
...
KBIRRunner::~KBIRRunner(){
  if( S_Agenda ) delete S_Agenda;
}

void KBIRRunner::runIR( WorkingMemory *wm ){
  /*
  Module *theM;
  ...
  */
  ...
  for( i = 0; i < domSuggests->getLength(); i++ ){
    S_Agenda->push( std::string(XMLString::transcode( ((DOMText *)(domSuggests->item(i)->getFirstChild()))->getData() )), S_wm );
  }
...

  {
	...
	Main->eraseFromParent();
      ...
      while( !S_Agenda->empty() ){
	call = theBuilder.CreateCall(theM->getFunction(S_Agenda->pop()),
				     args.begin(),
				     args.end(), "suggest");
	res = 
