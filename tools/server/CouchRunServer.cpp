...
const int CouchRunService::SERVER_STOPPED;
const int CouchRunService::SERVER_INITED;
const int CouchRunService::SERVER_RUNNING;
...
/** The working memory
 */
static CouchServerMemory *S_cmem = NULL;
...
/** The Agenda.
 */
CouchServerAgenda *CouchRunService::S_Agenda;

extern "C"
int ag_ppeval( char* txt ){
  std::string goal( txt );
  int ret = CouchRunService::S_Agenda->push( goal, S_cmem );
  std::cout << "Agenda: Forwarding hypothesis: " << goal << std::endl;
  // std::cout << *CouchRunService::S_Agenda;
  return ret;
}
...
CouchRunService::  CouchRunService( std::string URL, std::string DB, std::string DOC ){
  CouchRunService::S_Agenda = new CouchServerAgenda();
  couchURL = URL; couchDB = DB; couchDoc = DOC; 
  current_state = SERVER_STOPPED;
}

CouchRunService::~CouchRunService(){
  if( NULL != CouchRunService::S_Agenda ) delete CouchRunService::S_Agenda;
}

DOMDocument *CouchRunService::parsePrologue( std::string S_PrologueXMLFilename ){
...
const int CouchRunService::status(){ 
  CouchServerMemory *cmem = new CouchServerMemory( couchURL, couchDB, couchDoc );
  CouchServerAgenda *agenda = new CouchServerAgenda();
  int n, p;
...
void CouchRunService::restart(){
  ...
  DOMDocument *fDom = parsePrologue( fPrologueFile );
  if( NULL == fDom ){
    return;
  }
  ...
  S_cmem = cmem;
  CouchRunService::S_Agenda->reset();
  ...
  for( i = 0; i < domSuggests->getLength(); i++ ){
    CouchRunService::S_Agenda->push( std::string(XMLString::transcode( ((DOMText *)(domSuggests->item(i)->getFirstChild()))->getData() )), S_cmem );
  }
  ...
  cmem->setAgenda( CouchRunService::S_Agenda );
...
void CouchRunService::next(){
  // Prepares LLVM JIT and pass manager
  InitializeNativeTarget();
  LLVMContext &Context = getGlobalContext();  
  
  CouchServerMemory *cmem = 
    new CouchServerMemory( couchURL, couchDB, couchDoc );
  ...
  S_cmem = cmem;
  CouchRunService::S_Agenda->reset();
  S_cmem->getAgenda( CouchRunService::S_Agenda );
  std::cout << "Session Started\n" << *CouchRunService::S_Agenda;

  {
    ...
    while( !CouchRunService::S_Agenda->empty() ){
      // Get top priority goal in agenda
      hypo = std::string( CouchRunService::S_Agenda->top() );
	...
	try{
	  ...
	  (void)CouchRunService::S_Agenda->pop();
	  S_cmem->setAgenda( CouchRunService::S_Agenda );
	...
	*/
	...
	if( state ){
	  ...
	  current_state = SERVER_RUNNING;
	  S_cmem->setAgenda( CouchRunService::S_Agenda );
	  std::cout << "Session suspended on: " << pending <<  "\n";
	  delete cmem;
	  S_cmem = NULL;
	  return;
	}
...
void CouchRunService::suggest( std::string hypo ){
  CouchServerMemory *cmem = new CouchServerMemory( couchURL, couchDB, couchDoc );
  CouchServerAgenda *agenda = new CouchServerAgenda();
  cmem->getAgenda( agenda );
  agenda->push( hypo );
...
void CouchRunService::volunteer( std::string sign, int val_bool ){
  CouchServerMemory *cmem = new CouchServerMemory( couchURL, couchDB, couchDoc );
  cmem->set( sign.c_str(), val_bool );
  delete cmem;
}

void CouchRunService::volunteer( std::string sign, double val_num ){
  CouchServerMemory *cmem = new CouchServerMemory( couchURL, couchDB, couchDoc );
  cmem->set( sign.c_str(), val_num );
  delete cmem;
}

void CouchRunService::volunteer( std::string sign, std::string val_str ){
  CouchServerMemory *cmem = new CouchServerMemory( couchURL, couchDB, couchDoc );
  cmem->set( sign.c_str(), val_str.c_str() );
  delete cmem;
}
