...
/** @brief @b Interpreter Persists working memory to CouchDB datastore (no different from CouchServerWorkingMemory). */
class CouchHttpWorkingMemory : public WorkingMemory {
public:
  static const int WM_KNOWN = 1;
  static const int WM_UNKNOWN = 0;

  /** Constructors */
  CouchHttpWorkingMemory();
  CouchHttpWorkingMemory( std::string URL, std::string DB, std::string DOC );
  /** Destructor */
  ~CouchHttpWorkingMemory();
  ...
  */
  ...
  /** Assigns boolean inferred value to data
  ...
  //** Root JSON object in the response from CouchDB */
