...
const int CouchRunHttpServer::SERVER_STOPPED;
const int CouchRunHttpServer::SERVER_INITED;
const int CouchRunHttpServer::SERVER_RUNNING;
...
const std::string CouchRunHttpServer::getPending(){
  return S_pending;
}
 ...
 *------------------------------------------------------------------------------
*/
extern void buildQuestion( std::string question );
...
/** The working memory
 */
static CouchHttpMemory *S_cmem = NULL;
...
/** The Agenda.
 */
CouchHttpAgenda *CouchRunHttpServer::S_Agenda;

extern "C"
int ag_ppeval( char* txt ){
  std::string goal( txt );
  int ret = CouchRunHttpServer::S_Agenda->push( goal, S_cmem );
  std::cout << "Agenda: Forwarding hypothesis: " << goal << std::endl;
  // std::cout << *CouchRunHttpServer::S_Agenda;
  return ret;
}
...
CouchRunHttpServer::  CouchRunHttpServer( std::string URL, std::string DB, std::string DOC ){
  CouchRunHttpServer::S_Agenda = new CouchHttpAgenda();
  couchURL = URL; couchDB = DB; couchDoc = DOC; 
  current_state = SERVER_STOPPED;
}

CouchRunHttpServer::~CouchRunHttpServer(){
  if( NULL != CouchRunHttpServer::S_Agenda ) delete CouchRunHttpServer::S_Agenda;
}
...
const int CouchRunHttpServer::status(){ 
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  CouchHttpAgenda *agenda = new CouchHttpAgenda();
  int n, p;
...
void CouchRunHttpServer::getEncyclopedia( std::map<std::string, std::string> *map_h, std::map<std::string, std::string> *map_s ){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  cmem->getEncyclopedia( map_h, map_s );
  delete cmem;
}

void CouchRunHttpServer::init(){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  cmem->clear();
...
void CouchRunHttpServer::restart(){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  ...
  CouchRunHttpServer::S_Agenda->reset();
  ...
  for( i = 0; i < domSuggests->getLength(); i++ ){
    CouchRunHttpServer::S_Agenda->push( std::string(XMLString::transcode( ((DOMText *)(domSuggests->item(i)->getFirstChild()))->getData() )), S_cmem );
  }
  ...
  cmem->setAgenda( CouchRunHttpServer::S_Agenda );
...
void CouchRunHttpServer::next(){
  // Prepares LLVM JIT and pass manager
  InitializeNativeTarget();
  LLVMContext &Context = getGlobalContext();  
  
  CouchHttpMemory *cmem = 
    new CouchHttpMemory( couchURL, couchDB, couchDoc );
  ...
  S_cmem = cmem;
  CouchRunHttpServer::S_Agenda->reset();
  S_cmem->getAgenda( CouchRunHttpServer::S_Agenda );
  std::cout << "Session Started\n" << *CouchRunHttpServer::S_Agenda;

  {
    ...
    while( !CouchRunHttpServer::S_Agenda->empty() ){
      // Get top priority goal in agenda
      hypo = std::string( CouchRunHttpServer::S_Agenda->top() );
	...
	try{
	  ...
	  (void)CouchRunHttpServer::S_Agenda->pop();
	  S_cmem->setAgenda( CouchRunHttpServer::S_Agenda );
...
void CouchRunHttpServer::suggest( std::string hypo ){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  CouchHttpAgenda *agenda = new CouchHttpAgenda();
  cmem->getAgenda( agenda );
  agenda->push( hypo );
...
void CouchRunHttpServer::volunteer( std::string sign, int val_bool ){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  cmem->sets( sign.c_str(), val_bool );
  delete cmem;
}

void CouchRunHttpServer::volunteer( std::string sign, double val_num ){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
  cmem->sets( sign.c_str(), val_num );
  delete cmem;
}

void CouchRunHttpServer::volunteer( std::string sign, std::string val_str ){
  CouchHttpMemory *cmem = new CouchHttpMemory( couchURL, couchDB, couchDoc );
