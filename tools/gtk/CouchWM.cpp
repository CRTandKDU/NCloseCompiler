...
const int CouchExtWorkingMemory::WM_UNKNOWN;
const int CouchExtWorkingMemory::WM_KNOWN;
json_t *CouchExtWorkingMemory::S_Root = NULL;
...
static size_t S_write_f( void *ptr, size_t size, size_t nmemb, void *userdata){
  ...
  if(!root){
    ...
    CouchExtWorkingMemory::S_Root = root;
    json_t *version;
...
//
//===----------------------------------------------------------------------===//
CouchExtWorkingMemory::CouchExtWorkingMemory(){
...
CouchExtWorkingMemory::CouchExtWorkingMemory( std::string URL, std::string DB, std::string DOC ) {
  couchURL = URL;
  couchDB = DB;
  couchDocument = DOC;
  curl = curl_easy_init();
...
void CouchExtWorkingMemory::clear(){
  ...
  if( CouchExtWorkingMemory::S_Root ){  
    json_object_set_new( CouchExtWorkingMemory::S_Root, "agenda", json_array() );
    json_object_set_new( CouchExtWorkingMemory::S_Root, 
			 "hypotheses", json_array() );
    json_object_set_new( CouchExtWorkingMemory::S_Root, "signs", json_array() );
    ...
    if( json_is_null( wm ) ){
      ...
      free( s );
      // Clear
      json_object_set_new( CouchExtWorkingMemory::S_Root, "wm", json_object() );
      update();
    }
  }
...
//
//===----------------------------------------------------------------------===//
void CouchExtWorkingMemory::init(){
  if( curl ){
    char text[ BUFSIZE ];
...
void CouchExtWorkingMemory::update(){
  if( curl ){
    char text[ BUFSIZE ];
    char *s;
    curl_easy_reset( curl );
    ...
    s = json_dumps( CouchExtWorkingMemory::S_Root, 0 );
...
//
//===----------------------------------------------------------------------===//
const int CouchExtWorkingMemory::knownp( const char *data ){
...
//
//===----------------------------------------------------------------------===//
void CouchExtWorkingMemory::set_json_t( const char *data, json_t *val ){
  CouchExtWorkingMemory::S_Root = NULL;
  init();
  if( CouchExtWorkingMemory::S_Root ){
    json_t *wm = json_object_get( CouchExtWorkingMemory::S_Root, "wm" );
...
void CouchExtWorkingMemory::set( const char *data, int val ){
  printf( "Assign boolean value %d to variable %s.\n", val, data );
  set_json_t( data, json_integer( val ) );
}


void CouchExtWorkingMemory::set( const char *data, double val ){
  printf( "Assign value %f to variable %s.\n", val, data );
  set_json_t( data, json_real( val ) );
}

void CouchExtWorkingMemory::set( const char *data, const char *val ){
...
//
//===----------------------------------------------------------------------===//
json_t *CouchExtWorkingMemory::get_helper( const char *data ){
  json_t *field = NULL;
  CouchExtWorkingMemory::S_Root = NULL;
  init();
  if( CouchExtWorkingMemory::S_Root ){
    field = json_object_get( CouchExtWorkingMemory::S_Root, data );
  }
  return field;
}

std::string CouchExtWorkingMemory::get_assembly( std::string def ){
...
std::string CouchExtWorkingMemory::get_prologue( std::string def ){
...
json_t *CouchExtWorkingMemory::get_json_t( char *data ){
  CouchExtWorkingMemory::S_Root = NULL;
  init();
  if( CouchExtWorkingMemory::S_Root ){
    json_t *wm = json_object_get( CouchExtWorkingMemory::S_Root, "wm" );
...
int CouchExtWorkingMemory::get( char *data ){
...
double CouchExtWorkingMemory::getd( char *data ){
  json_t *sign = get_json_t( data );
...
const char * CouchExtWorkingMemory::getc( char *data ){
  json_t *sign = get_json_t( data );
...
std::ostream& operator<<(std::ostream& out, const CouchExtWorkingMemory& wmem){
  return out;
}
